...
// THE MEASURED FLUXES (141st pass): good-flag hours that carry the
// bulk inputs AND a directly measured flux - the covariance
// (eddy-correlation) stress, sensible and latent heat fluxes, the
// inertial-dissipation fluxes, and where the ship's laser
// altimeter ran, the significant wave height and dominant period.
// PSL's own bulk fluxes ride along for reference. Sampled at a
// fixed stride through all covariance hours, plus a finer stride
// through the wave hours (the code's wave-state branch is tried
// on those). Written to its own file (shipflux-cov-fixture.js) so
// the skin fixture stays the size the 137th chose.
const covQ =
  'cruise_name,time,latitude,tair,qair,z_tair,z_qair,wspd_sfc,z_wspd,tskin,psealevel,ssea_ship,wspd_10N,ustar,tau_bulk,tau_cov,tau_id,hs_bulk,hs_cov,hs_id,hl_bulk,hl_cov,hl_id,wave_height,wave_period&flag_bad_bulk=0&flag_bad_ship=0';
const covAllRows = await fetchCsv(covQ);
const covNeed = [
  'latitude',
  'tair',
  'qair',
...
const covEligible = covAllRows.rows.filter(
  (o) =>
    fin(o, covNeed) &&
    o.wspd_sfc > 0 &&
    ((Number.isFinite(o.tau_cov) && o.tau_cov > 0) ||
      Number.isFinite(o.hs_cov) ||
      Number.isFinite(o.hl_cov))
);
const waveEligible = covEligible.filter(
  (o) =>
    Number.isFinite(o.wave_height) &&
    Number.isFinite(o.wave_period) &&
    Number.isFinite(o.tau_cov) &&
    o.tau_cov > 0
);
const STRIDE_COV = 12;
const STRIDE_WAVE = 4;
const covPickSet = new Set([
  ...covEligible.filter((_, i) => i % STRIDE_COV === 0),
  ...waveEligible.filter((_, i) => i % STRIDE_WAVE === 0)
]);
const covPick = covEligible.filter((o) => covPickSet.has(o));
const opt = (x, f = r3) => (Number.isFinite(x) ? f(x) : null);
const cov = covPick.map((o) => ({
  cruise: o.cruise_name,
  time: o.time,
  latDeg: r3(o.latitude),
  taC: r3(o.tair),
  ...
  tskinC: r4(o.tskin),
  pHpa: r3(o.psealevel),
  ssPsu: opt(o.ssea_ship),
  u10nMs: opt(o.wspd_10N),
  uStar: opt(o.ustar, r4),
  tauBulk: opt(o.tau_bulk, r4),
  tauCov: opt(o.tau_cov, r4),
  tauId: opt(o.tau_id, r4),
  hsDown: opt(o.hs_bulk),
  hsCov: opt(o.hs_cov),
  hlDown: opt(o.hl_bulk),
  hlCov: opt(o.hl_cov),
  waveHm: opt(o.wave_height),
  waveTpS: opt(o.wave_period)
}));

// cruise names once; rows carry an index (a third of the bytes)
const cruiseList = [
  ...new Set(
    [...skin, ...allsky, ...clear, ...warm, ...cov].map((o) => o.cruise)
  )
];
const cruises = new Set(cruiseList);
for (const set of [skin, allsky, clear, warm, cov])
  for (const o of set) {
...
const outCov = `/**
 * shipflux-cov-fixture.js - the MEASURED FLUXES of NOAA PSL's hourly
 * ship archive (NOAA_PSL_Hourly_Ship_Flux on the COAPS ERDDAP),
 * GENERATED by shipflux-freeze.mjs at ${at} for the 141st pass:
 * good-flag hours (flag_bad_bulk 0, flag_bad_ship 0) that carry
 * the bulk inputs and a directly measured flux - the covariance
 * stress along the wind (tau_cov), the covariance sensible and
 * latent heat fluxes (the archive: sensible from the sonic
 * temperature covariance with the humidity part removed by the
 * bulk latent flux; the latent fluxes corrected for gas-analyser
 * bias after Fratini et al. 2014), the inertial-dissipation
 * stress, and where the ship's laser altimeter ran (Riegl LD90),
 * the significant wave height and dominant period. PSL's own bulk
 * values ride along (uStar, tauBulk, hsDown, hlDown - downward-
 * positive as the archive serves them; the covariance and ID
 * fluxes likewise). Systematic samples in time order: every
 * ${STRIDE_COV}th of ${covEligible.length} eligible hours, plus every ${STRIDE_WAVE}rd of the
 * ${waveEligible.length} hours with waves and a covariance stress (${cov.length} rows,
 * ${cruises.size} cruises). Rows carry a cruise index c into
 * SHIPFLUX_CRUISES (shipflux-fixture.js, the same list). Generated
 * data, one row per line - listed in .prettierignore.
 */
export const SHIPFLUX_COV_AT = '${at}';
export const SHIPFLUX_COV_QUERY = ${JSON.stringify(covAllRows.url)};
export const SHIPFLUX_COV_COUNTS = ${JSON.stringify(
  {
    goodRows: covAllRows.rows.length,
    eligible: covEligible.length,
    stride: STRIDE_COV,
    waveEligible: waveEligible.length,
    waveStride: STRIDE_WAVE
  },
  null,
  2
)};
export const SHIPFLUX_COV = ${packed(cov)};
`;
const pathCov = resolve(HERE, 'shipflux-cov-fixture.js');
writeFileSync(pathCov, outCov);
console.log(
  `wrote ${pathCov}: cov ${cov.length} rows (${covEligible.length} eligible, ${waveEligible.length} with waves), ${(outCov.length / 1024).toFixed(0)} kB`
);
