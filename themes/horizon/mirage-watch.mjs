 ...
 *
 *   node mirage-watch.mjs [--json hits.json] [--freeze]
 *
 * THE STANDING SESSION PROCEDURE (132nd pass): run
 *   node mirage-watch.mjs --freeze
 * once per session. Closing days not already in the archive are
 * appended to lehn-fixture.js with their run-then-pinned bands;
 * then READ THE FIXTURE DIFF, run lehn-reference (the archive
 * runner holds every day to its own pins), validate, commit.
 *
...
const doFreeze = args.includes('--freeze');
...
if (doFreeze && hits.length) {
  // Append closing days the archive lacks, with their bands
  // written NOW (run-then-pin: the deliberateness lives in
  // reading the fixture diff before committing).
  const fx = await import('./lehn-fixture.js').catch(() => ({DAYS: []}));
  const have = new Set((fx.DAYS ?? []).map((d) => d.station.name + '|' + d.at));
  const days = [...(fx.DAYS ?? [])];
  let added = 0;
  for (const h of hits) {
  ...
  if (added) {
    ...
    writeFileSync(
      './lehn-fixture.js',
      header + 'export const DAYS = ' + JSON.stringify(days, null, 1) + ';\n'
    );
    console.log(
      `archive: ${days.length} day(s); READ THE DIFF, run lehn-reference, validate, commit`
    );
  } else {
    console.log('archive: every closing day already frozen');
  }
}
