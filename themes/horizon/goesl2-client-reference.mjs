...
// minute (the timed asks), then for a point no bucket reaches.
{
  ...
  check(
    ...
    body !== null &&
      ...
      // one before for the six the fake bucket lists nothing under
      ...
      asked.size === 0,
    `the home asks ${L2_ASKS.length} products over ${listsAfterFirst} listings (this hour's prefix for each, two more hours back for the six found empty) and reads the two the fake bucket holds ` +
...
// products are asked again.
{
  ...
  check(
    ...
    a.length === 100 &&
      ...
      again.sst === null &&
      again.dsr === null &&
      // the CONUS products re-listed (three prefixes for the four the
