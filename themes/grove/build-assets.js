...
// tail; bounds through the grass and stands bolt upright to periscope (faces right). in winter it
// moults to a white ermine — keeping that black tail-tip — so the same shapes take either coat ========
const mustelidDef = (id, c0, c1) =>
  `<defs><linearGradient id="${id}" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="${c0}"/><stop offset="1" stop-color="${c1}"/></linearGradient></defs>`;
// c0/c1 = back gradient, base = tail root + ears, leg = limb strokes, belly = underside (tip stays black)
const stoatLow = (id, c0, c1, base, leg, belly, ear) =>
  `${mustelidDef(id, c0, c1)}
<path d="M30,52 C20,52 11,50 5,46" stroke="${base}" stroke-width="6.5" fill="none" stroke-linecap="round"/>
<path d="M10,49 C7,48 4,47 2,45" stroke="#1c1a17" stroke-width="6.5" fill="none" stroke-linecap="round"/>
<g stroke="${leg}" stroke-width="4" stroke-linecap="round"><line x1="38" y1="55" x2="36" y2="66"/><line x1="68" y1="55" x2="70" y2="66"/></g>
<path d="M28,52 C30,38 46,33 62,36 C76,38 82,46 78,53 C66,58 42,58 28,52 Z" fill="url(#${id})"/>
<path d="M40,55 C54,58 70,56 78,51 C70,57 52,58 40,56 Z" fill="${belly}"/>
<ellipse cx="80" cy="44" rx="9" ry="7.5" fill="url(#${id})"/>
<circle cx="76" cy="37.5" r="2.8" fill="${ear}"/><circle cx="84" cy="38.5" r="2.4" fill="${ear}"/>
<path d="M81,49 C85,49 87,47 87,45 C85,49 83,50 81,50 Z" fill="${belly}"/>
<circle cx="83" cy="42" r="1.5" fill="#100c08"/><circle cx="89" cy="44" r="1.8" fill="#171310"/>`;
const stoatUp = (id, c0, c1, base, leg, belly, ear) =>
  `${mustelidDef(id, c0, c1)}
<path d="M44,60 C34,62 29,66 33,70" stroke="${base}" stroke-width="6" fill="none" stroke-linecap="round"/>
<path d="M35,68 C33,69 31,69 30,69.5" stroke="#1c1a17" stroke-width="6" fill="none" stroke-linecap="round"/>
<g stroke="${leg}" stroke-width="4" stroke-linecap="round"><line x1="46" y1="64" x2="44" y2="70"/><line x1="55" y1="64" x2="57" y2="70"/></g>
<path d="M41,68 C37,52 41,33 49,27 C57,22 65,29 64,42 C63,54 59,64 53,68 C49,70 44,70 41,68 Z" fill="url(#${id})"/>
<path d="M49,66 C46,52 48,37 53,31 C53,44 53,56 56,66 C53,68 51,68 49,66 Z" fill="${belly}"/>
<ellipse cx="51" cy="21" rx="8" ry="7" fill="url(#${id})"/>
<circle cx="46" cy="15.5" r="2.8" fill="${ear}"/><circle cx="56" cy="15.5" r="2.8" fill="${ear}"/>
<ellipse cx="55" cy="40" rx="3" ry="4.5" fill="url(#${id})"/>
<path d="M51,26 C54,26 56,24 56,22 C55,26 53,27 51,27 Z" fill="${belly}"/>
...
// the winter ermine: white coat shaded a touch cool, pale ears and limbs, and the black tail-tip kept
save(
  'ermine',
  S(
    '96 72',
