...
# Chrome page download button spoof

The chrome download page checks download host URL parameter with bypassable `startsWith`.
```js
Se = "https://dl.google.com"

s = e.get(C.FALLBACK_INSTALLER_PATH) && e.get(C.FALLBACK_INSTALLER_PATH).startsWith(Se) ? e.get(C.FALLBACK_INSTALLER_PATH) : this.storage_.getItem(A.INSTALLER_PATH);
```
