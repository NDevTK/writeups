...
- User gets automatically redirected to an attacker's captive portal (Normal secure non-Google-looking website).
- User taps 'accept cookies' for popup (User activation).

# Attacker script steps:

- The attacker page then does two things: it opens a popup and redirects the now-hidden tab to `http://unsafe.codeassist.google.com/` which is over an insecure connection and controlled by the attacker.
