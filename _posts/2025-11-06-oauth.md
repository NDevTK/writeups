...
- Attacker controlled `http://unsafe.codeassist.google.com/` uses its post message listener to steal the secret OAuth URL via `window.onmessage`
  > GitHub doesn't support the implicit grant type but it's likely abused by using the <https://codeassist.google.com/api/finishoauth> method same as the previous VRP report.
...
- Google Vulnerability Reward Program panel decided to issue a reward of $400.00 category is S2: Vulnerabilities requiring pre-existing exploited vulnerabilities to execute. Default Google Cloud products. We downgraded the amount to 80% due to this report not meeting the criteria for a good quality report Nov 18, 2025 02:55PM
- Google Vulnerability Reward Program panel decided to issue a reward of $669.60 category is Insecure by default: updated to Tier 1 based on the notebooks.cloud.google.com redirect Nov 25, 2025 02:55PM
